...
export function AdminMobileMenu({ slug }: { slug: string }) {
  ...
  // Portal solo es válido en cliente. Evitamos hydration mismatch.
  ...
  return (
    <>
      ...
      {/* El drawer va al body via portal. Si está dentro de un padre con
          backdrop-filter/transform/filter, fixed se ata a ese padre en
          lugar del viewport y el drawer no cubre toda la pantalla. */}
      {mounted && createPortal(
        <>
          {/* Backdrop dimmer */}
          <div
            className={`fixed inset-0 z-[100] sm:hidden transition-opacity duration-200 ${
          ...
          {/* Drawer */}
          <div
            className={`fixed top-0 right-0 h-full w-80 max-w-[88vw] z-[110] border-l border-white/[0.08] transition-transform duration-300 sm:hidden ${
