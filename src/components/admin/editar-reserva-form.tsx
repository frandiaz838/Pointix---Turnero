...
import { editarReserva } from "@/actions/reservas"
...
export function EditarReservaForm({ slug, canchas, ocupacion, reservaInicial }: Props) {
  const [pending, startTransition] = useTransition()
  const [error, setError] = useState<string | null>(null)

  const [courtId, setCourtId]       = useState(reservaInicial.courtId)
  const [fechaDate, setFechaDate]   = useState<Date>(dateFromIso(reservaInicial.fecha))
  const [hora, setHora]             = useState(reservaInicial.hora)
  const [guestName, setGuestName]   = useState(reservaInicial.guestName)
  const [guestPhone, setGuestPhone] = useState(reservaInicial.guestPhone)
  const [guestEmail, setGuestEmail] = useState(reservaInicial.guestEmail)
  ...
  // Para edición, excluimos el slot original de la lista de ocupados
  // (la reserva actual no está bloqueando "su propio" horario)
  const horasOcupadas = useMemo(() => {
    if (!courtId) return new Set<string>()
    const key = `${courtId}|${isoFromDate(fechaDate)}`
    const set = new Set(ocupacionMap.get(key) ?? new Set<string>())
    if (courtId === reservaInicial.courtId && isoFromDate(fechaDate) === reservaInicial.fecha) {
      set.delete(reservaInicial.hora)
    }
    return set
  }, [courtId, fechaDate, ocupacionMap, reservaInicial])
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        await editarReserva(reservaInicial.id, formData)
      } catch (err) {
